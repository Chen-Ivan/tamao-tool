...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
		...
		// null 处理
		writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new TamaoBeanSerializerModifier()));
		writeObjectMapper.getSerializerProvider().setNullValueSerializer(TamaoBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
